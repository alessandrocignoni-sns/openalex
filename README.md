...
- **251106_DatiWorksInCSVDaCSV.py** - From a CSV file in the same folder of the script containing name, surname, fiscal code and ORCID id of authors, first it uses the authors endopoint to get the OpenAlex id of the author, secondly it gets the data for each author work, then it writes it in a CSV file containing surname-name string, OpenAlex id of the work, DOI, title and publishing year of each work. It also prints the fiscal code of authors not registered in OpenAlex.
- **251127_DatiWorksAnniInCSVDaCSV.py** - From a CSV file in the same folder of the script containing name, surname, fiscal code and ORCID id of authors, first it uses the authors endopoint to get the OpenAlex id of the author, secondly it gets data for each author work in a given years frame, then it writes it in a CSV file containing surname-name string, DOI, title, publishing year, type of publication and biblographical datas of each work. It also prints the fiscal code of authors not registered in OpenAlex.
